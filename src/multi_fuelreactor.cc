...
MFReactor::MFReactor(cyclus::Context* ctx)
    : cyclus::Facility(ctx),
      n_assem_batch(0),
      assem_size(0),
      n_assem_core(0),
...
#pragma cyclus def clone cycamore::MFReactor

#pragma cyclus def schema cycamore::MFReactor

#pragma cyclus def annotations cycamore::MFReactor

#pragma cyclus def infiletodb cycamore::MFReactor

#pragma cyclus def snapshot cycamore::MFReactor

#pragma cyclus def snapshotinv cycamore::MFReactor

#pragma cyclus def initinv cycamore::MFReactor

void MFReactor::InitFrom(MFReactor* m) {
  #pragma cyclus impl initfromcopy cycamore::MFReactor
  cyclus::toolkit::CommodityProducer::Copy(m);
}

void MFReactor::InitFrom(cyclus::QueryableBackend* b) {
  #pragma cyclus impl initfromdb cycamore::MFReactor
...
bool MFReactor::CheckDecommissionCondition() {
...
std::set<cyclus::RequestPortfolio<Material>::Ptr> MFReactor::GetMatlRequests() {
...
void MFReactor::AcceptMatlTrades(const std::vector<
...
std::set<cyclus::BidPortfolio<Material>::Ptr> MFReactor::GetMatlBids(
...
void MFReactor::Transmute() { Transmute(n_assem_batch); }

void MFReactor::Transmute(int n_assem) {
...
std::map<std::string, MatVec> MFReactor::PeekSpent() {
...
std::string MFReactor::fuel_incommod(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_incommods.size()) {
    throw KeyError("cycamore::MFReactor - no incommod for material object");
  }
  return fuel_incommods[i];
}

std::string MFReactor::fuel_outcommod(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_outcommods.size()) {
    throw KeyError("cycamore::MFReactor - no outcommod for material object");
  }
  return fuel_outcommods[i];
}

std::string MFReactor::fuel_inrecipe(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_inrecipes.size()) {
    throw KeyError("cycamore::MFReactor - no inrecipe for material object");
  }
  return fuel_inrecipes[i];
}

std::string MFReactor::fuel_outrecipe(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_outrecipes.size()) {
    throw KeyError("cycamore::MFReactor - no outrecipe for material object");
  }
  return fuel_outrecipes[i];
}

double MFReactor::fuel_pref(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
...
void MFReactor::index_res(cyclus::Resource::Ptr m, std::string incommod) {
  ...
  throw ValueError(
      "cycamore::MFReactor - received unsupported incommod material");
}

std::map<std::string, MatVec> MFReactor::PopSpent() {
  MatVec mats = spent.PopN(spent.count());
  std::map<std::string, MatVec> mapped;
  for (int i = 0; i < mats.size(); i++) {
    std::string commod = fuel_outcommod(mats[i]);
...
void MFReactor::PushSpent(std::map<std::string, MatVec> leftover) {
  std::map<std::string, MatVec>::iterator it;
...
void MFReactor::Record(std::string name, std::string val) {
  context()
      ->NewDatum("MFReactorEvents")
      ->AddVal("AgentId", id())
...
extern "C" cyclus::Agent* ConstructMFReactor(cyclus::Context* ctx) {
  return new MFReactor(ctx);
}
